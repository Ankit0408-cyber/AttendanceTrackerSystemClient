  ...
  const handleAddEmployee = async (e) => {
    ...
    try {
      const payload = {
        ...
        salaryPerDay: newEmployee.role === 'employee' ? Number(newEmployee.salaryPerDay) : undefined,
      };
  ...
  const handleUpdateUser = async (e) => {
    ...
    try {
      const payload = {
        ...
        salaryPerDay: editForm.role === 'employee' ? Number(editForm.salaryPerDay) : undefined,
      };
      ...
      setEditForm({ name: '', email: '', password: '', department: '', salaryPerDay: '', role: 'employee' });
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <form onSubmit={handleAddEmployee} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label htmlFor="salaryPerDay" className="block text sm font-medium text-gray-700">
                  Daily Salary Rate (₹){' '}
                  {newEmployee.role === 'employee' && <span className="text-xs text-gray-500">(required)</span>}
                </label>
                <input
                  type="number"
                  id="salaryPerDay"
                  value={newEmployee.salaryPerDay}
                  onChange={(e) => setNewEmployee({ ...newEmployee, salaryPerDay: e.target.value })}
                  className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-red-500 focus:border-red-500"
                  min="0"
                  step="0.01"
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <form onSubmit={handleUpdateUser} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label htmlFor="edit-salaryPerDay" className="block text sm font-medium text-gray-700">
                  Daily Salary Rate (₹){' '}
                  {editForm.role === 'employee' && <span className="text-xs text-gray-500">(required)</span>}
                </label>
                <input
                  type="number"
                  id="edit-salaryPerDay"
                  value={editForm.salaryPerDay}
                  onChange={(e) => setEditForm({ ...editForm, salaryPerDay: e.target.value })}
                  className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-red-500 focus:border-red-500"
                  min="0"
                  step="0.01"
                ...
                <div key={emp._id} className={`flex items-center justify-between p-4 rounded-lg border ${
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                      ...
                      {emp.salaryPerDay && ` • Daily Rate: ₹${emp.salaryPerDay}`}
                    </p>
                  </div>
                  <div className="flex items-center space-x-2">
                    <button
